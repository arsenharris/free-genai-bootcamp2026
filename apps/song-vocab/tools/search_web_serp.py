try:
    ...
    GoogleSearch = None
    # Defer import errors to runtime; we'll handle missing client gracefully
...
async def search_web_serp(query: str, max_results: int = 5) -> List[Dict[str, str]]:
    ...
    spanish_keywords = ["letra", "letras", "lyrics", "español"]
    enhanced_query = f"{query} {' '.join(spanish_keywords)}"
    logger.info(f"Enhanced query: {enhanced_query}")
    
    try:
        # If serpapi isn't installed or import failed, return empty list
        if GoogleSearch is None:
            logger.warning("serpapi package not available; returning no search results")
            return []

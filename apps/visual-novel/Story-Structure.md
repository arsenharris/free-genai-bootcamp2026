# Visual Novel Story Structure (Spanish learning)
...
When a current dialog node is set it becomes a state.
- speaker
- response
...
Sometimes a choice shows a short response but still follows the `default_next_id`. Example:
```json
{
  "id": "scene002",
  "dialog": {
...
### Study Path Endings:
- **Reading/Writing Ending**: Player excels in written Spanish and passes an intermediate-level exam (e.g., DELE A2/B1 equivalent)
...
Each scene includes:
- **Relevant Vocabulary**: Words and phrases specific to the location and situation (e.g., `Correos`, `tienda`, `metro`)
- **Grammar Points**: New structures introduced through character dialog (present/preterite/imperfect, pronouns, formal vs informal)
- **Cultural Notes**: Insights into Spanish customs, greetings, and etiquette
- **Practice Exercises**: Interactive language challenges with feedback (fill-in-the-blanks, choose the correct register, roleplay responses)
